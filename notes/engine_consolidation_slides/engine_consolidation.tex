...
\setbeamertemplate{footline}{%
  ...
  \begin{beamercolorbox}[wd=.78\paperwidth,ht=2.5ex,dp=1.1ex,leftskip=1em]{author in head/foot}%
    JaxCont v0.2 engine consolidation
  \end{beamercolorbox}%
...
\title{How JaxCont Follows Solution Branches}
\subtitle{Pseudo-arclength continuation, the scan engines, and the JAX performance strategy}
\author{Beginner-friendly technical presentation}
\date{Merged state: \texttt{main} at \texttt{b105417}, 22 July 2026}
...
\begin{enumerate}
  ...
  \item What \texttt{pseudo\_arclength\_scan} actually means in JaxCont.
  \item Why fixed buffers, \texttt{lax.while\_loop}, JIT and \texttt{vmap} make it fast.
  \item What the consolidated v0.2 engine does today, and what it still cannot do.
\end{enumerate}
...
The consolidated engine uses Newton effort as a difficulty signal:
\begin{center}
\begin{tabular}{lll}
\toprule
...
\section{What the scan engines are}
...
\begin{frame}{Why the engine was consolidated}
Before consolidation, three implementations duplicated the same predictor--corrector idea:
\begin{center}
\begin{tikzpicture}[node distance=7mm,>=Latex,
 old/.style={rounded corners,draw=JaxRed,fill=JaxRed!8,minimum width=43mm,minimum height=9mm,align=center},
 issue/.style={rounded corners,draw=JaxOrange,fill=JaxOrange!10,align=center}]
\node[old] (n) {\texttt{NaturalContinuation}\\Python-loop OO};
\node[old,below=of n] (p) {\texttt{PseudoArclengthContinuation}\\Python-loop OO};
\node[old,below=of p] (s) {\texttt{pseudo\_arclength\_scan}\\compiled functional};
\node[issue,right=18mm of p] (bug) {Fixes and behavior drifted\\between implementations};
\draw[->,thick,JaxRed] (n.east)--(bug.west);
\draw[->,thick,JaxRed] (p.east)--(bug.west);
\draw[->,thick,JaxRed] (s.east)--(bug.west);
\end{tikzpicture}
\end{center}
A real example: autodiff/bordered-solve fixes landed in pseudo-arclength but not in the sibling natural-continuation class. Duplication had become a correctness risk.
\end{frame}

\begin{frame}{Consolidated architecture}
\centering
...
\end{tikzpicture}
\vspace{3mm}
Both engines return the same fixed-shape \texttt{ScanResult}; natural continuation zero-fills the tangent buffer because it has no tangent concept.
\end{frame}
...
\begin{frame}{The original bottleneck was dispatch, not algebra}
...
\begin{alertblock}{Measured lesson from JaxCont}
JIT-compiling only the corrector did not measurably improve small-system continuation: roughly $21$ ms/point before and after. The Python outer loop remained dominant.
\end{alertblock}
\begin{block}{Optimization target}
Reduce dispatch count by compiling the entire predictor--corrector sweep, not merely one small kernel inside it.
\end{block}
\end{frame}
...
\begin{tikzpicture}[>=Latex,
 py/.style={draw=JaxRed,fill=JaxRed!8,rounded corners,minimum width=18mm,minimum height=8mm,align=center},
 xla/.style={draw=JaxGreen,thick,fill=JaxGreen!10,rounded corners,
             minimum width=118mm,minimum height=14mm,align=center}]
\node[anchor=west,font=\bfseries,text=JaxRed] at (-5.5,1.30)
    {Before: Python coordinates and dispatches every step};
\node[py] (p1) at (-4.4,.45) {Python\\step 1};
\node[py] (p2) at (-2.2,.45) {Python\\step 2};
\node[py] (p3) at (0,.45) {Python\\step 3};
\node[py] (pd) at (2.2,.45) {$\cdots$};
\node[py] (pm) at (4.4,.45) {Python\\step $M$};
\draw[->] (p1)--(p2); \draw[->] (p2)--(p3); \draw[->] (p3)--(pd); \draw[->] (pd)--(pm);

\node[anchor=west,font=\bfseries,text=JaxGreen!60!black] at (-5.5,-.62)
    {After: one device program owns the complete loop};
\node[xla] (all) at (0,-1.55)
    {\textbf{one compiled XLA program}\\[1mm]
     tangent $\rightarrow$ predict $\rightarrow$ Newton $\rightarrow$ adapt $\rightarrow$ repeat};
\end{tikzpicture}
\vspace{2mm}
...
\begin{frame}{Performance evidence---and how to state it honestly}
Repository measurements report:
\begin{itemize}
  \item pitchfork scan: about $0.74$ ms warm versus about $250$ ms for the old Python loop (roughly $340\times$ in that experiment);
  \item 256-diagram \texttt{vmap} example: about $163\times$ versus a Python loop in the recorded run;
  \item CPU and local RTX A5000 smoke tests execute full continuation and batched sweeps.
\end{itemize}
\begin{alertblock}{Do not generalize the numbers}
Speedup depends on model size, precision, device, branch length, compilation cache and comparison baseline. Report cold time, warm time, hardware and settings separately.
\end{alertblock}
The defensible claim is architectural: fewer dispatches, compiled control flow, vectorized ensembles and accelerator-compatible arrays.
\end{frame}

\begin{frame}{Where the time and memory still go}
\begin{itemize}
...
\end{itemize}
\begin{block}{Future scaling seam}
The roadmap proposes explicit linear/eigen-solver protocols so dense solves can later be replaced by iterative or matrix-free methods without redesigning the continuation API.
\end{block}
\end{frame}

\section{Correctness, scope, and presentation}

\begin{frame}{How the consolidation is tested}
\begin{itemize}
  \item Direct tests of tangent and bordered Newton mathematics.
  \item Natural scan tracks a linear branch and stalls at a fold as designed.
  \item Pseudo-arclength passes folds and preserves tangent orientation.
  \item Adaptive-step tests check fast/slow/failure behavior and per-point $\Delta s$.
  \item \texttt{vmap} tests check fixed shapes, masks and batch outputs.
  \item Functional API tests verify both algorithms dispatch to scan engines.
  \item Cross-validated gallery examples compare with analytic results and BifurcationKit.jl references.
\end{itemize}
\begin{block}{Final verification recorded at completion}
All 75 default tests and 12 slow-marked tests passed with zero failures. All six
migrated gallery examples ran headlessly, and the BifurcationKit.jl matches in
examples 02 and 05 remained intact.
\end{block}
\end{frame}

\begin{frame}{What the merged consolidation changed}
\begin{itemize}
  \item added \texttt{natural\_scan} and per-point step-size buffers;
  \item routed \texttt{Natural()} and \texttt{PseudoArclength()} through shared scan reassembly;
  \item removed three old OO/Python-loop continuation modules and free-function facades;
  \item migrated all 32 dependent tests, all six affected gallery examples, and the profiling script;
  \item repaired the user guide, quickstart, API reference, and development documentation;
  \item merged the completed work, design, and implementation plan into \texttt{main}.
\end{itemize}
\begin{alertblock}{Scope}
This is engine consolidation, a prerequisite for v0.2 periodic orbits. It does not itself implement periodic-orbit collocation, Floquet multipliers or the new event protocol.
\end{alertblock}
\end{frame}

\begin{frame}{What remains after engine consolidation}
\begin{itemize}
  \item Event detection is not transformation-safe and has known duplicate/spurious classifications in difficult examples.
  \item Per-point Newton iteration counts are not yet carried into the high-level metadata.
  ...
  \item The two scan bodies still duplicate their predict--correct--write--adapt--stop skeleton; this was deliberately deferred until a third predictor justifies an abstraction.
\end{itemize}
\begin{block}{Healthy engineering message}
The consolidation is complete and merged. Periodic orbits, solver protocols,
and a transformation-safe event protocol remain separate roadmap work.
\end{block}
\end{frame}
...
\begin{enumerate}
  ...
  \item The performance breakthrough is whole-loop compilation; JIT on tiny inner pieces was insufficient.
  \item Fixed buffers, masks and array control flow are the price of JIT/\texttt{vmap}/GPU compatibility.
  \item Engine consolidation reduces correctness risk and creates one functional spine for future periodic-orbit work.
  \item Fast does not remove numerical responsibilities: convergence, conditioning, validation and honest limitations still matter.
\end{enumerate}
\end{frame}
...
\small
\begin{itemize}
  \item \texttt{docs/superpowers/specs/2026-07-21-engine-consolidation-design.md}
  \item \texttt{docs/superpowers/plans/2026-07-21-engine-consolidation.md}
  \item merged implementation in \texttt{src/jaxcont/core/scan\_continuation.py}
  \item public dispatch and result paths in \texttt{src/jaxcont/api.py}
  \item \texttt{notes/ARCHITECTURE.md}, \texttt{notes/ROADMAP.md}, and beginner guide
  \item branch tests and examples, especially natural/PALC, \texttt{vmap}, and differentiable fold examples
  \item final verification in \texttt{notes/ROADMAP.md} and commit history through \texttt{b105417} on \texttt{main}
\end{itemize}
\begin{block}{Reproducibility note}
Performance numbers are repository measurements, not universal benchmarks. Re-run them on the target hardware before external publication.
\end{block}
\end{frame}
