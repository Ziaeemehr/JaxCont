...
\begin{frame}{The public objects hide one compiled branch engine}
\centering
\begin{tikzpicture}[node distance=8mm and 10mm,>=Latex,
 ...
 engine/.style={rounded corners,draw=JaxGreen,very thick,fill=JaxGreen!10,
                minimum width=38mm,minimum height=13mm,align=center}]
\node[box] (model) {\texttt{rhs, u0, p0, args}};
\node[box,right=of model] (problem) {\texttt{BifProblem}};
\node[box,right=of problem] (call) {algorithm + settings\\\texttt{continuation(...)}};
\node[engine,below=of call] (loop) {bounded whole-loop\\JAX engine};
\node[box,left=of loop] (result) {\texttt{ContinuationResult}};
\draw[->,thick] (model)--(problem);
\draw[->,thick] (problem)--(call);
\draw[->,thick] (call)--(loop);
\draw[->,thick] (loop)--(result);
\end{tikzpicture}
\vspace{7mm}
\begin{block}{Why the boundary matters}
Users compose immutable problem, algorithm, settings, and event values. The
engine can change its carry representation or post-processing without changing
how demos construct a problem or read a branch.
\end{block}
\vfill
