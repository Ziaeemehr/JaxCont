...
\node[inspect,left=of small] (inspect) {inspect / visualize\\[1mm]
  residuals; \texttt{n\_valid}/mask; termination\\
  step size; stability; event diagnostics};
...
\textbf{Run:} \texttt{MPLBACKEND=Agg python -}, paste that Python block, then send EOF.\\
\textbf{Expected:} \texttt{[('fold', 0.0)]} and an annotated branch figure; a
...
figures (the script does not save them).\\
...
\node[box,below left=7mm and 3mm of stop] (yes) {Inspect endpoint coverage and stop
evidence: residuals, finite values, \texttt{ds\_min}, the \texttt{max\_steps}
accepted-point budget, and extra rejected-attempt work.};
\node[box,below right=7mm and 3mm of stop] (no) {Inspect stability and event evidence:
eigenvalues or Floquet multipliers, detector diagnostics, and local refinement residuals.};
\node[final,below=7mm of yes] (important) {\textbf{Important conclusion:}
rerun with changed step bounds, resolution, and direction.};
\node[final,below=7mm of no] (publish) {\textbf{Publication claim:}
...
The public result exposes coverage, residual and validity evidence, but not a
single explicit termination-reason field; compare the last accepted point with
the configured endpoint and stop conditions.
\end{alertblock}
\end{frame}
